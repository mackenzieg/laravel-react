...
/**
 * Modified by Mackenzie to allow for child rendering
 */
...
class ReactJS {
  ...
  private
    ...
    $children = null,
//    $children = [
//      'example.ExampleComponent' => ['example', 'data',
//        ['example.AnotherComponent' => ['more', 'data']
//        ]
//      ],
//    ],
   ...
   */
  function setComponent($component, $data = null, $children = null) {
  ...
  function getMarkup() {
    $js = sprintf(
      ...
      {$this->exploreChildren($this->children)})))",
      $this->component,
      $this->data);
   ...
   * @param array $current traverse down child tree
   * @return string
   */
  private function exploreChildren($current) {
    if (!$current) {
      return null;
    }
    return sprintf('React.createElement(%s, %s, %s)',
      $current['component'],
      $current['data'],
      $this->exploreChildren($current['children']));
  }
